...
def slider(window,clock = None,
    ...
    next = visual.ImageStim(window,image=pngPath+'Next.jpeg', units = 'norm', pos=(0,-0.9),size=.1)
        ...
        if slider.contains(x,y) or (snap2mouse and sliderLine.contains(x,y)):
            ...
            while mouse.getPressed()[0] == 1:
                ...
                if next.contains(x,y):
                    ...
                    next.setImage(pngPath+'Next.jpeg')
            vrange = limits[1]-limits[0]
            if clickFirst == False:
                value = -999
            else:
...
def scale(window,clock = None,
    ...
    next = visual.ImageStim(window,image=pngPath+'Next.jpeg', size=1,units = 'norm', pos=(0,-0.9))
...
def Distribution(window,clock = None,
                drawList = [],
                width = 0.8,
                height = 1.2,
                labels = None,
    ...
    def val2pos(val=None):
        range = float(maxPos-base)
        return (((val-limits[0])/(limits[1]-limits[0]))*range)+base

    def pos2val(pos=None):
        range = float(limits[1]-limits[0])
        return ((pos)/(maxPos-base))*range
    ...
    if labels:
        ...
        if intervals == 0:      # If only one label was given, position it in the middle
            labelStims.append(visual.TextStim(window,units = 'norm', text=labels[0], alignHoriz='center', alignVert='top', color=labelColors[0], height=h, pos=(0,-0.7)))
        else:
            ...
            for label in labels:
                ...
                labelStims.append(visual.TextStim(window,units = 'norm', text= labels[count], alignHoriz='center', alignVert='top', color=labelColors[count], height=h, pos=(pos,-0.75)))
                count += 1
    ...
    if MaxVal:
        intervals = MaxVal - 1
        if intervals == 0:      # If only one label was given, position it in the middle
            tickers.append(visual.TextStim(window,units = 'norm', text='', color=yLabelColors[0], height=0.05, alignHoriz='right',pos=(-width-.01,base+(maxPos-base)/2.0)))
        else:
            spacing = (maxPos-base)/intervals
            count = 0
            for label in range(MaxVal):
                pos = base + (spacing * count)
                tickers.append(visual.TextStim(window,units = 'norm', text='', height=0.05, alignHoriz='right', pos=(-width-.01,pos)))
                count += 1
    ...
    for i in range(len(bars)):
        trackers.append(visual.TextStim(window,text = '%s'%val,height=.04,pos=[bars[i].pos[0],-.62],color='black'))

    for l in range(len(labels)-1):
        labelStims[l].pos[0] = bars[l].pos[0]
    # create axes
    if drawAxes:
    ...
    next = visual.ImageStim(window,image=pngPath+'right_arrow.png', units = 'norm', pos=(0,-0.9),size=.1)

    mouse = event.Mouse(win=window)     # create the mouse object for responding to clicks
    Tot = visual.TextStim(window,text='%s'%MaxVal,pos=[0,.7],color='black')
    ...
    total = 0
    for i in range(len(bars)):
        total += int(pos2val(bars[i].vertices[1][1]))
    if MaxVal - total < 0:
        Ttotal = MaxVal-np.floor(total)
    elif MaxVal - total > 0:
        Ttotal = MaxVal - np.ceil(total)
        DisplayTot = 100-Ttotal
    Tot.setText('%s'%int(DisplayTot))
    ...
    #               while mouse.getPressed()[0] == 1:
                    if MaxVal > 0:
                        ...
                        if mouse.getPressed()[0] == 1:
                            ...
                            setting = max(0.0,min(maxPos-base, -1*(base-newY)))
                            setting = nearestLabel(val2pos(pos2val(setting)))-base
                            vertices[1][1] = setting
                            vertices[2][1] = setting
                            bars[index].setVertices(vertices)
                            trackers[index].setPos([bars[index].pos[0],vertices[1][1]+base+.03])
                            trackers[index].setText('%s'%int(pos2val(bars[index].vertices[1][1])))
                            drawAll()
                            Tot.draw()
                            window.flip()
                            total = 0
                            ...
                            if total > 100:
                                ...
                                Tot.setText('%s'%int(total))
                            if MaxVal - total - 1 >= 0:
                                nextVisible = False
                            else:
                                nextVisible = True

        if nextVisible and next.contains(x,y) and Ttotal >= 0:
            # If the subject clicked on the Next button, then darken it and track motion while the mouse is down.
            # If they move the mouse off the Next button before releasing it, lighten the button etc.
            # When they finally release the mouse button, consider the Next button clicked only if the mouse
            # is currently on it..
            clickTime = clock.getTime()
            next.setImage(pngPath+'right_arrow.png')
    ...
    Steps = outputLabels
    for i in range(len(bars)):
        vv.append(int(pos2val(bars[i].vertices[1][1])))
    rt = 0
    vv.append(timedOut)
    return settings,rt,vv
...
class Question():
    def __init__(self,idex,window, vPos = 0.0,amount = .25,labelSize = .05,labelColor = 'black'):
        self.Selected = False
        self.Bselected = False
        self.Gselected = False
        self.window = window
        self.vPos = vPos
        self.amount = amount
        ...
        aspectRatio = (float(self.window.size[1])/float(self.window.size[0]))       # This is needed when scaling the radio buttons so they stay round

        self.Number =  visual.TextStim(self.window,units = 'norm', text='%s.'%(self.idex+1), alignHoriz = 'left', height=self.labelSize, color = self.labelColor, pos=(-.75,self.vPos))
        self.SureBet = visual.TextStim(self.window,units = 'norm', text='I choose the sure amount of %s Tokens.'%self.amount, alignHoriz = 'left', height=self.labelSize, color = self.labelColor, pos=(-.6,self.vPos))
        rbSize=labelSize
        self.buttonS = visual.ImageStim(self.window,image=pngPath+'rb.png', size=(rbSize*aspectRatio,rbSize),units = 'norm', pos=(-.64,self.vPos-self.labelSize*.05))

        self.Gamble = visual.TextStim(self.window,units = 'norm', text='I choose to pick a ball.', alignHoriz = 'left', height=self.labelSize, color = self.labelColor, pos=(.3,self.vPos))
        rbSize=labelSize    # The radio buttons look too big if not scaled down relative to the labels
        self.buttonG = visual.ImageStim(self.window,image=pngPath+'rb.png', size=(rbSize*aspectRatio,rbSize),units = 'norm', pos=(.26,self.vPos-self.labelSize*.05))


    def Update(self):
        mouse = event.Mouse(win=self.window)
        self.SureBet.draw();self.buttonS.draw()
        self.Gamble.draw();self.buttonG.draw();self.Number.draw()
        x, y  = mouse.getPos()
        if rbClicked(x,y,self.buttonS): #and mouse.getPressed()[0] == 1:
            self.Bselected = True
            self.Gselected = False
        elif rbClicked(x,y,self.buttonG): #and mouse.getPressed()[0] == 1:
            self.Bselected = False
            self.Gselected = True

        if self.Bselected:
            self.buttonS.setImage(pngPath+'rbc.png')
            self.buttonG.setImage(pngPath+'rb.png')
            self.Selected = True
        elif self.Gselected:
            self.buttonS.setImage(pngPath+'rb.png')
            self.buttonG.setImage(pngPath+'rbc.png')
            self.Selected = True
        elif (not self.Bselected) and (not self.Gselected):
            self.buttonS.setImage(pngPath+'rb.png')
            self.buttonG.setImage(pngPath+'rb.png')

    def Output(self,Type):
        # 0 = sure bet, 1 = gamble
        if self.Bselected: return [Type, self.idex, self.amount, 0]
        if self.Gselected: return [Type, self.idex, self.amount, 1]


def RiskAmbig(window,num,path,riskAmb = 'Risk'):
    optA = visual.TextStim(window,units = 'norm',text = 'Option A',alignHoriz = 'left',height=.05, color = 'black', pos=(-.4,.9))
    optB = visual.TextStim(window,units = 'norm',text = 'Option B',alignHoriz = 'left',height=.05, color = 'black', pos=(.37,.9))
    try:
        Into = open('%s/RiskAmb.csv'%path,mode = 'r').readlines()
        Output = open('%s/RiskAmb.csv'%path,mode = 'w')
        for i in Into:
            Output.write(i)
    except:
        Output = open('%s/RiskAmb.csv'%path,'w')

    List = []
    vPos = .8
    amount = .25
    for i in range(num):
        List.append(Question(i,window,vPos,amount = amount))
        vPos -= .085
        amount += .25
    next = visual.ImageStim(window,image=pngPath+'right_arrow.png', units = 'norm', pos=(0,-0.9),size=.1)
    nextVisible = False
    ...
    while not done:
        ...
        Allselected = 0
        for x in List:
            if x.Selected: Allselected += 1
        if Allselected == num:
            next.draw()
            nextVisible = True
        x,y = mouse.getPos()

        if nextVisible and next.contains(x,y) and mouse.getPressed()[0] == 1:
            next.setImage(pngPath+'right_arrow.png')
            next.draw()
            window.flip()
            for i in List:
                outy = i.Output(riskAmb)
                for x in outy[:-1]:
                    Output.write('%s,'%x)
                Output.write('%s\n'%outy[-1])
            Output.close()
...
def choice(window,clock = None,
    ...
    while (not done):
        ...
        if nextVisible and event.getKeys(['right']):
            rt = 0
            done = True
    window.flip()
    return selected, rt
...
def textInput(window, clock=None,
    ...
    global message
    message = ''
    next = visual.ImageStim(window,image=pngPath+'right_arrow.png', size=.1,units = 'norm', pos=(0,-0.9))
...
def form(window, clock=None,
    ...
    for field in fields:
        ...
        fieldpos = [x, y]
        y -= (size + 0.03)
        formFields.append(Field(window, label=label, maxChars=max, pos=fieldpos, type=type, text=text,size = size))
    ...
    while (not done):
        ...
        if event.getKeys('tab'):
            fieldIndex +=1
            if fieldIndex >= len(formFields):
                fieldIndex = 0
            activeField = formFields[fieldIndex]
        if event.getKeys(['right','return']):
            rt = clock.getTime() - startTime
            done = True
    window.winHandle.on_text = orig_on_text
    del window.winHandle.on_text_motion
